...
{
    internal static void MakePedDropWeapon(Ped ped)
    {
        bool dropWeapon = Settings.DoesPedDropWeapon && (int)EntryPoint.GenerateChance() <= Settings.Chance;
        if (dropWeapon && ped.Exists() && ped.IsAlive && NativeFunction.Natives.x475768A975D5AD17<bool>(ped, 1 | 2 | 4)) // IS_PED_ARMED
        {
